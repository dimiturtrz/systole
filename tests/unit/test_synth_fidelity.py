...
def test_dprime_separated_vs_overlapping():
    torch.manual_seed(0)
    far = dprime(torch.randn(2000), torch.randn(2000) + 4.0)     # 4 SD apart -> d' ~ 4
    same = dprime(torch.randn(2000), torch.randn(2000))          # same dist -> d' ~ 0
...
def test_dprime_affine_invariant():
    ...
    base = dprime(a, b)
    assert abs(dprime(a * 7.3 - 1.5, b * 7.3 - 1.5) - base) < 0.02


def test_dprime_tiny_sample_is_nan():
    v = dprime(torch.randn(10), torch.randn(3000))              # < 50 pts -> unstable -> NaN
    assert v != v
