...
from cardioseg.evaluation.distribution import _groups, _pooled, lo_hi, strata_table
...
def test_pooled_empty_is_empty_arrays():
    ...
    dists, dice_acc, ef_gt, ef_pred = _pooled([])
...
def test_lo_hi_formats_signed_band():
    ...
    assert lo_hi(0.0, 0.0) == "+0.0, +0.0"
    assert lo_hi(2.0, 1.0) == f"{2.0 - LOA_Z:+.1f}, {2.0 + LOA_Z:+.1f}"
