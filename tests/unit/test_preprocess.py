...
def test_fit_square_already_square():
    ...
    np.testing.assert_array_equal(fit_square(a, 4), a)
...
def test_fit_square_smaller_is_centre_padded():
    ...
    out = fit_square(a, 6, pad_value=-1.0)
...
def test_stack_slices_casts_and_stacks():
    ...
    out = stack_slices(slices, 5, dtype=np.uint8)
...
def test_blood_anchor_falls_back_to_zscore():
    ...
    np.testing.assert_allclose(blood_anchor(img, gt), zscore(img), rtol=1e-5)
...
def test_preprocess_case_zscore_default():
    ...
    out = preprocess_case("/tmp/patientX", _fake_loader, target_inplane=1.5)
...
def test_preprocess_case_blood_norm_and_missing_frame():
    ...
    out = preprocess_case("/tmp/pY", _ed_only, norm="blood")
...
def test_preprocess_case_with_n4():
    ...
    out = preprocess_case("/tmp/pN", _fake_loader, n4=True)
...
def test_preprocess_case_with_nyul(monkeypatch):
    ...
    def _fake_nyul(img, standard):
        seen["called"] = True
        return img
    monkeypatch.setattr(pp, "nyul_transform", _fake_nyul)
    out = pp.preprocess_case("/tmp/pNy", _fake_loader, nyul_standard=[0.0, 0.5, 1.0])
