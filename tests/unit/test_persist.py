...
def test_prov_paper_layer_wins():
    ...
    p = _prov("field_scanner", {"field_scanner": "auto-src"},
...
def test_prov_auto_rest_fallback():
    ...
    assert _prov("x", {"rest": "sidecar"}, {})["source"] == "sidecar"
    assert _prov("x", {}, {})["source"] == "unknown"
...
def test_main_single_dataset(monkeypatch):
    ...
    called = []
    monkeypatch.setattr(P, "persist_meta", lambda ds: called.append(ds))
