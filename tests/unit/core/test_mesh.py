...
def test_chamber_surface_produces_mesh():
    mesh = Mesh.chamber_surface(_blob(3), 3, spacing=(1.0, 1.0, 1.0))
    assert mesh is not None and mesh.n_points > 0 and mesh.n_faces > 0


def test_chamber_surface_absent_is_none():
    assert Mesh.chamber_surface(_blob(3), 1, spacing=(1.0, 1.0, 1.0)) is None   # label 1 not present
...
def test_chamber_surface_tiny_is_none():
    ...
    assert Mesh.chamber_surface(m, 3, spacing=(1.0, 1.0, 1.0)) is None
...
def test_chamber_surface_coarse_resample_none():
    ...
    assert Mesh.chamber_surface(m, 3, spacing=(1.0, 1.0, 1.0), iso=20.0) is None


def test_export_stl_writes_per_chamber(tmp_path):
    m = _blob(3).copy(); m[6:18, 6:18, 6:18] = 3     # a clean single chamber
    paths = Mesh.export_stl(m, (1.0, 1.0, 1.0), tmp_path, "subjX")
...
def test_export_glb_writes_file(tmp_path):
    ...
    m = _blob(3)
    out = Mesh.export_glb(m, (1.0, 1.0, 1.0), tmp_path / "s.glb")
...
def test_export_meshes_both_formats(tmp_path):
    ...
    m = _blob(3)
    out = Mesh.export_meshes(m, (1.0, 1.0, 1.0), "subjZ", root=tmp_path)
...
def test_export_meshes_glb_only(tmp_path):
    ...
    out = Mesh.export_meshes(_blob(3), (1.0, 1.0, 1.0), "subjG", formats=("glb",), root=tmp_path)
...
def test_main_cli_reads_npz_and_exports(monkeypatch, tmp_path):
    """main: parses argv, loads the npz, dispatches export_meshes with the chosen frame/root."""
