...
def test_geom_metrics_are_sane():
    g = StaticCompare.geom_metrics(_mask())
    ...
    assert StaticCompare.geom_metrics(np.zeros((16, 16), np.uint8)) is None       # empty -> None


def test_compare_zero_for_identical_grows_on_shift():
    same = [_mask() for _ in range(20)]
    w1_same = StaticCompare.compare(same, same)
    ...
    w1_shift = StaticCompare.compare(same, bigger_lv)
