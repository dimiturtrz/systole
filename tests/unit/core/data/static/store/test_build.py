...
def test_build_process_if_missing(tmp_path, monkeypatch):
    ...
    normalized.clear()
    out2 = Build.build("acdc", 1.5)                              # second call: both present
...
def test_build_rebuild_forces_all(tmp_path, monkeypatch):
    ...
    Build.build("acdc", 1.5)
    Build.build("acdc", 1.5, rebuild=True)
...
def test_load_cfg_threads_all_params(monkeypatch):
    ...
    def _fake_load(names=None, **kw):
        captured.update(names=names, **kw)
    ...
    assert captured["inplane"] == 1.25 and captured["n4"] is True
    assert captured["n4_params"] == d.n4_params
    assert captured["nyul"] is False and captured["norm"] == "blood" and captured["workers"] == 3
...
def test_load_cfg_sources_override(monkeypatch):
    ...
    monkeypatch.setattr(build_mod.Build, "load",
                        staticmethod(lambda names=None, **kw: captured.update(names=names) or pl.DataFrame({"x": [1]})))
