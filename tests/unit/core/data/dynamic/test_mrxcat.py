...
def test_unmapped_codes_are_background():
    ...
    assert (Mrxcat.to_canonical(raw) == 0).all()


def test_empty_and_allbg_are_zero():
    assert Mrxcat.to_canonical(np.zeros((4, 4), int)).sum() == 0
...
def test_place_heart_in_fov_swaps_anatomy():
    ...
    out = Mrxcat.place_heart_in_fov(fov, heart)
...
def test_fovbg_paints_wholefov_map():
    ...
    sq = Mrxcat._heart_crop_scale(big, size=128, target_px=80)
...
def test_fov_window_crops_chest_window():
    ...
    out = Mrxcat._fov_window(s, size=64, scale=3.0)
...
def test_build_ssm_fov_pool_no_backgrounds_empty(tmp_path):
    ...
    _, shape = Mrxcat.build_ssm_fov_pool(rp, tmp_path, tmp_path / "ssm_fov.npz", size=32)
...
def test_build_ssm_fov_pool_composites(vti_dir):
    ...
    _, shape = Mrxcat.build_ssm_fov_pool(rp, vti_dir, vti_dir / "ssm.npz", size=48, scale=3.0)
