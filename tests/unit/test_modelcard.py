...
"""
from cardioseg.evaluation.modelcard import _perf_table, reference_rows, render_card
...
def test_render_card_has_core_sections_and_run_name():
    ...
    md = render_card("myrun", _cfg(), _metrics())
...
def test_render_card_includes_test_section_when_present():
    ...
    assert "### Held-out test" in render_card("r", _cfg(), _metrics(with_test=True))


def test_render_card_omits_test_section_when_absent():
    """Test-absent class: val-only metrics -> no held-out-test heading."""
    assert "### Held-out test" not in render_card("r", _cfg(), _metrics(with_test=False))
...
def test_render_card_appends_reference_section():
    ...
    md = render_card("r", _cfg(), _metrics(), ref_section=["", "## Reference ranges (derived from our GT, for context)"])
