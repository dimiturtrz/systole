...
class Inference:
    """Static inference primitives: model + [D,H,W] volume -> label map / softmax / member stack."""
    ...
    def predict_volume_members(model, vol_img: Volume, size: int, device: str):
        ...
        model.eval()
        xs = Inference._stack_slices(vol_img, size)
        with torch.no_grad():
    ...
    def predict_volume_probs(model, vol_img: Volume, size: int, device: str):
        ...
        pred, mean, _ = Inference.predict_volume_members(model, vol_img, size, device)
        return pred, mean
    ...
    def predict_volume(model, vol_img: Volume, size: int, device: str, *, tta: bool = False) -> Volume:
        ...
        if tta:
            pred, _ = Inference.predict_volume_probs(model, vol_img, size, device)
            return pred
        model.eval()
        xs = Inference._stack_slices(vol_img, size)
        with torch.no_grad():
