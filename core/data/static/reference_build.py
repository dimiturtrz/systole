"""Reference BUILD-side — derive reference values from processed/ (ground-truth masks + real images)
and emit the machine-dimension acquisition table. Split out of reference.py so the read-side loader
(reference.py) no longer imports store: this breaks the core.data.static.store <-> reference cycle
(store's nyul/acquisition lookups import reference.py, which used to import store back here)."""
...
class ReferenceBuild:
    """Builder CLI that mines reference values from processed/ — derives EF/volume ranges + per-vendor
    per-class intensity levels from GROUND TRUTH, and emits the machine-dimension acquisition table from
    physics. Computed from GROUND TRUTH, so model-independent + reproducible (the "data is scarce -> trace
    ...
    def build_from_store(sources=None, inplane: float | None = None, out_dir: str | Path | None = None) -> Path:
        ...
        for path, g in sorted(by_path.items()):
            ...
            out["ef_by_pathology"][path] = ReferenceBuild._range_entry(g["ef"], base)
            out["volumes"][f"{path}_edv_ml"] = ReferenceBuild._range_entry(g["edv"], base)
            out["volumes"][f"{path}_esv_ml"] = ReferenceBuild._range_entry(g["esv"], base)
        ...
        header = ("# DERIVED reference ranges — generated by `python -m core.data reference --build` from the\n"
    ...
    def build_real_levels(sources=None, inplane: float | None = None, per_case: int = 400,
        ...
        header = ("# MEASURED per-vendor per-class intensity levels (z-units) — generated by\n"
                  "# `python -m core.data reference --real-levels` from the store's real images.\n"
    ...
    def build_acquisition(out_dir: str | Path | None = None) -> Path:
        ...
        header = ("# MACHINE DIMENSION — cine bSSFP acquisition per (vendor, field). Generated by\n"
                  "# `python -m core.data reference --acquisition`. LOCAL (not committed).\n"
