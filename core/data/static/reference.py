...
def build_acquisition(out_dir: str | Path | None = None) -> Path:
    """Emit the MACHINE-DIMENSION artifact `<data>/reference/acquisition.yaml` from the committed cine-
    bSSFP paper priors (`mri_physics._ACQ`). Vendor-keyed; flip split per field (the SAR-driven axis),
    TR/TE shared. All leaves verified:false — inert until a human verifies them (e.g. DICOM-mines the
    volumes); until then `acquisition_for` falls back to the same committed priors.

    NORMALIZED: this is the machine reference table (attributes live once). Subjects hold the FK
    (vendor, field_T) in the store and JOIN here via `acquisition_for` — acquisition is never copied per
    subject. (bd ex1 / 276; source = deep-dive 2026-07-01_mri-vendor-acquisition-params.)"""
    from core.data.dynamic.mri_physics import _ACQ, _ACQ_DEFAULT
    cites = {
        "tr": "cross-vendor cine bSSFP prior ~3ms (mriquestions; SCMR 2020 PMC7038611)",
        "te": "TE~=TR/2 ~1.3ms (mriquestions)",
        "flip_1p5t": "1.5T cine flip 45-80deg (Kuetting 2018 PubMed 28429574; mriquestions)",
        "flip_3t": "3T SAR cap <=50deg, opt 30-40deg (Wang/Nayak PubMed 26509846; PMC6570530)",
    }
    def leaf(v, src):
        return {"value": v, "source": src,
                "based_on": "deep-dive 2026-07-01_mri-vendor-acquisition-params",
                "extracted_by": "paper", "verified": False}
    acq = {}
    for v in sorted({vn for vn, _ in _ACQ}):
        tr, te, f15 = _ACQ.get((v, 1.5), _ACQ_DEFAULT)
        f30 = _ACQ.get((v, 3.0), _ACQ.get((v, 1.5), _ACQ_DEFAULT))[2]
        acq[v] = {"tr_ms": leaf(tr, cites["tr"]), "te_ms": leaf(te, cites["te"]),
                  "flip_deg_1p5t": leaf(f15, cites["flip_1p5t"]),
                  "flip_deg_3t": leaf(f30, cites["flip_3t"])}

    base_dir = Path(out_dir) if out_dir is not None else reference_dir()
    base_dir.mkdir(parents=True, exist_ok=True)
    path_out = base_dir / "acquisition.yaml"
    header = ("# MACHINE DIMENSION — cine bSSFP acquisition per (vendor, field). Generated by\n"
              "# `python -m core.data.static.reference --acquisition` from the committed paper priors\n"
              "# (mri_physics._ACQ). LOCAL (not committed). ALL leaves verified:false = inert until a human\n"
              "# verifies them (DICOM-mine the volumes); acquisition_for falls back to the same priors.\n"
              "# Subjects JOIN this by (vendor, field_T) — acquisition is never copied per subject.\n")
