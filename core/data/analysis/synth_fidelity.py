...
class SynthFidelity:
    ...
    def _pair_dprime(x1c: torch.Tensor, ym: torch.Tensor, n_classes: int) -> dict:
        ...
        for i, j in _PAIRS:
            if i < n_classes and j < n_classes:
                out[f"{_NAMES[i]}|{_NAMES[j]}"] = round(SynthFidelity.dprime(x1c[ym == i], x1c[ym == j]), 3)
        return out
    ...
    def separability(X: torch.Tensor, Y: torch.Tensor, cfg, n_classes: int, device: str) -> dict:
        ...
        def per_slice(x1c):
            ...
            for i in range(x1c.shape[0]):
                for k, v in SynthFidelity._pair_dprime(x1c[i].reshape(-1), ym[i].reshape(-1), n_classes).items():
                    acc.setdefault(k, []).append(v)
            return {k: round(float(np.nanmean(v)), 3) for k, v in acc.items()}
        out = {}
        for lvl, real_v, syn_v in (("pooled", SynthFidelity._pair_dprime(Xr.reshape(-1), ym.reshape(-1), n_classes),
                                    SynthFidelity._pair_dprime(Xs.reshape(-1), ym.reshape(-1), n_classes)),
    ...
    def real_spread_bands(meta, n_classes: int, size: int, max_per_vendor: int = 800) -> dict:
        ...
        for v in sorted(meta.get_column("vendor").unique().to_list()):
            ...
            if n > max_per_vendor:
                ...
                X, Y = X[idx], Y[idx]
            per_vendor[v] = SynthFidelity._spread(X[:, 0], Y, n_classes)["pooled"]
        band = {}
        for c in range(n_classes):
    ...
    def distance(self, X: torch.Tensor, Y: torch.Tensor, q: int = 100) -> dict:
        ...
        for c in range(self.n_classes):
            m = ym == c
            r, s = rx[m], sx[m]
            dist[_NAMES[c]] = round(self.wasserstein1d(r, s, q), 3)
            if r.numel() and s.numel():
                loc[_NAMES[c]] = round(abs(float(r.mean()) - float(s.mean())), 3)
                shape[_NAMES[c]] = round(self.wasserstein1d(r - r.mean(), s - s.mean(), q), 3)
            else:
