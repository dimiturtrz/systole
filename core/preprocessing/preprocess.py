...
class Preprocess:
    """The per-subject preprocessing transform (free functions folded in as staticmethods; public
    ...
    def blood_anchor(img: Image, gt, blood=(1, 3), eps: float = ZSCORE_EPS) -> Image:
        ...
        if int(bm.sum()) < _MIN_ANCHOR_PX or int(am.sum()) < _MIN_ANCHOR_PX:
            return Preprocess.zscore(img)                        # no anchor available -> safe fallback
    ...
    def preprocess_case(  # noqa: PLR0913  independent preprocessing inputs
        ...
        for tag in ("ED", "ES"):
            ...
            gt, _ = Preprocess.resample_inplane(d[tag]["gt"], sp, target_inplane, is_mask=True)
            if n4:
