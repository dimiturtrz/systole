...
def resolve_data_root(raw: str, *, os_name: str | None = None, wsl: bool | None = None) -> str:
    ...
    wsl = detect_wsl() if wsl is None else wsl
    drive = win_drive(raw)
    if os_name == "nt":
        return to_windows(raw) if (drive is None and mnt_drive(raw)) else raw
    # POSIX
    if drive is None:
        return raw                                # already a posix path
    if wsl:
        return to_wsl(raw)                         # D:/x -> /mnt/d/x
    raise RuntimeError(
