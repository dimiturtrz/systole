...
- **graph.py** — import-graph arch fitness (god-module = fan-in AND fan-out over a degree / import cycle / god-file / **test-mirror gap**). Gate: `python -m devtools.graph --assert <packages>` (thresholds in `pyproject [tool.structure]`). Also computes **Martin's instability** `I = Ce/(Ce+Ca)` and **main-sequence distance** `D = |A + I − 1|` (abstractness `A` from the class AST) — reported in the explorer, and an **advisory** gate (`main_sequence_max`, OFF at 0 — a concrete stable leaf legitimately sits at `D ≈ 1`, so it's opt-in). Explorer: drop `--assert` for ranked fan-in/out/bottleneck/chokepoint/instability/distance tables. `--assert --no-test-mirror` gates structure (god-module/cycle/god-file) on a legitimately test-less tree (e.g. a bag of single-file CLI tools) without demanding a per-module mirror test.
